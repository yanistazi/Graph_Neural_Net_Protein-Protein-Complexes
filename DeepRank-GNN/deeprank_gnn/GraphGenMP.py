...
class GraphHDF5(object):
    ...
    def __init__(self, pdb_path, ref_path=None, graph_type='residue', pssm_path=None,
                 ...
                 contact_distance=8.5, internal_contact_distance=3): # Yanis cont distance and internal cont distance
        ...
        if nproc == 1:
            ...
            contact_distance=contact_distance, internal_contact_distance=internal_contact_distance) # Yanis cont and int cont distance)
    ...
    def get_all_graphs(self, pdbs, pssm, ref, outfile, use_tqdm=True, biopython=False,
                      contact_distance=8.5, internal_contact_distance=3): # Yanis cont and int cont distance)
        ...
        for name in lst:
            try:
                graphs.append(self._get_one_graph(name, pssm, ref, biopython,contact_distance=contact_distance, internal_contact_distance=internal_contact_distance))  # Yanis cont and int cont distance
            except Exception as e:
                print('Issue encountered while computing graph ', name)
                print(e)
    ...
    def _pickle_one_graph(name, pssm, ref, tmpdir='./', biopython=False,
        ...
        # get the graph
        g = ResidueGraph(pdb=name, pssm=pssm[name], biopython=biopython,contact_distance=contact_distance, internal_contact_distance=internal_contact_distance)  # Yanis cont and int cont distance
        if ref is not None:
            g.get_score(ref)
