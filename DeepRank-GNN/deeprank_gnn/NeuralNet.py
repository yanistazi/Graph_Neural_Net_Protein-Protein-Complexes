    ...
    def __init__(self, database, Net,
        ...
        if pretrained_model is None:
            ...
            if self.task == 'reg' and self.threshold == None:
                ...
                self.threshold = 0.3
            self.load_model(database, Net, database_eval,train_using_pretrained_model=False)

        else:
            self.load_params(pretrained_model)
            self.outdir = outdir
            self.optimizer_name = optimizer_name # Yanis add optimizer name
            if train_using_pretrained_model == False :
                self.load_pretrained_model(database, Net)
            else :
                self.load_model(database, Net, database_eval,train_using_pretrained_model=True)
...
#             self.model.parameters(), lr=self.lr)
        ...
        self.optimizer = eval("torch.optim."+self.optimizer_name+"(self.model.parameters(), lr="+str(self.lr)+")")  #Yanis to change optimizer
        print(self.model,self.optimizer)
        # load the model and the optimizer state if we have one
        self.optimizer.load_state_dict(self.opt_loaded_state_dict)
        self.model.load_state_dict(self.model_load_state_dict)

    def load_model(self, database, Net, database_eval,train_using_pretrained_model=False):
        """
        Loads model
...
#             self.model.parameters(), lr=self.lr)
        ...
        if train_using_pretrained_model : #Yanis
            self.optimizer.load_state_dict(self.opt_loaded_state_dict)
            self.model.load_state_dict(self.model_load_state_dict)
            print("Loading pretrained model and continue_training/transfer learning")
        else:
    ...
    def set_loss(self):
        ...
        if self.task == 'reg':
            self.loss = MSELoss(reduction="sum")  # Yanic change sum so that later we divide by len(data)
    ...
    def train(self, nepoch=1, validate=False, save_model='last', hdf5='train_data.hdf5', save_epoch='intermediate', save_every=5,plot_every=5,plot_name="plot_loss",epoch_start=0,keep_latest_saved_models=1000,keep_latest_saved_plots=1000):
        """
        Trains the model
        ...
        # Loop over epochs
        self.data = {}
        for epoch in range(epoch_start+1, epoch_start+nepoch+1):
            ...
            if validate is True:
                ...
                if save_model == 'best':
                    ...
                    if min(self.valid_loss) == _val_loss:
                        self.save_model(filename=self.outdir+'t{}_y{}_b{}_e{}_lr{}_{}.pth.tar'.format(
            ...
            if (epoch % plot_every == 0):
                self.plot_loss(self.outdir + plot_name+'_t{}_y{}_b{}_e{}_lr{}_{}.png'.format(
                    self.task, self.target, str(self.batch_size), str(nepoch), str(self.lr), str(epoch)),nepoch=epoch,
                              epoch_start=epoch_start)
                np.savetxt(self.outdir+"train_loss.txt",self.train_loss) # Yanis keep track of train loss
                np.savetxt(self.outdir+"valid_loss.txt",self.valid_loss) # Yanis keep track of validation loss
            
            # Keep only N latest saved models and latest n plot loss
            try:
                files_model = glob.glob(self.outdir+"*.pth.tar")
                files_model.sort(key=os.path.getmtime)
                if len(files_model)>keep_latest_saved_models:
                    os.remove(files_model[0])
            except:
                print("Error removing saved models")
                  
            try:                        
                files_plot = glob.glob(self.outdir+"*.png")
                files_plot.sort(key=os.path.getmtime)
                if len(files_plot)>keep_latest_saved_plots:
                    os.remove(files_plot[0])
            except:
                print("Error removing saved plots")
                                       
        # Save the last model
        if save_model == 'last':
            self.save_model(filename=self.outdir+'t{}_y{}_b{}_e{}_lr{}.pth.tar'.format(
...
#         return out, y, loss_val, data   
        return out, y, loss_val/len(loader.dataset), data     #Yanis change
...
#         return out, y, running_loss, data
        return out, y, running_loss/len(self.train_loader.dataset), data # Yanis change
    ...
    def plot_loss(self, name='',nepoch="",epoch_start=0):
        """
        ...
        if len(valid_loss) > 1:
#             plt.plot(range(1, nepoch+1), valid_loss[2:len(,
#                      c='red', label='valid')
            plt.plot(range(epoch_start+3, nepoch+1), valid_loss[2:len(valid_loss)],
                     c='red', label='valid') # Yanis to fix scaling at epoch 1 start plotting at epoch 3.

        if len(train_loss) > 1:
#             plt.plot(range(1, nepoch+1), train_loss,
#                      c='blue', label='train')
            plt.plot(range(epoch_start+3, nepoch+1), train_loss[2:len(train_loss)],
                     c='blue', label='train')   # Yanis to fix scaling at epoch 1 start plotting at epoch 3.
